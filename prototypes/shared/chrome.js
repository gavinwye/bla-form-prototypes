...
// Each prototype passes its full folder name as the key, e.g. 'motor-vehicle-inspection'
...
function setupRadioGroup(groupName, onSelect) {
  const buttons = document.querySelectorAll(`[data-group="${groupName}"]`);
  buttons.forEach(btn => {
    btn.addEventListener('click', () => {
      buttons.forEach(b => { b.setAttribute('aria-checked', 'false'); b.innerHTML = ''; });
      btn.setAttribute('aria-checked', 'true');
      btn.innerHTML = '<span style="width:1.5rem;height:1.5rem;border-radius:9999px;background:#000;display:block;flex-shrink:0;"></span>';
...
function getRadioValue(groupName) {
  const sel = document.querySelector(`[data-group="${groupName}"][aria-checked="true"]`);
...
// Standard text input HTML builder (returns HTML string)
function textInputHTML({ id, label, hint, type = 'text', inputmode, placeholder, width = 'w-full' }) {
  return `
    <div class="flex flex-col gap-xs ${width} items-start">
      <p id="${id}-error" class="hidden text-[1.1rem] text-bb-red-00 font-bold" role="alert"></p>
      ...
      <div id="${id}-wrapper" class="relative inline-flex ${width} rounded-sm border-2 border-bb-black-00 items-center gap-2 transition-all bg-bb-white-00 hover:shadow-form-hover focus-within:ring-4 focus-within:ring-bb-teal-100">
        <input type="${type}" id="${id}" name="${id}" ${inputmode ? `inputmode="${inputmode}"` : ''} ${placeholder ? `placeholder="${placeholder}"` : ''} class="w-full min-w-0 p-s outline-none rounded-[inherit] placeholder:text-bb-black-00/60" />
      </div>
    </div>`;
}

// Standard radio option HTML
function radioOptionHTML(groupName, value, label) {
  return `
    <div class="flex gap-5 items-center">
      <button type="button" role="radio" aria-checked="false" data-group="${groupName}" data-value="${value}"
        class="relative inline-flex items-center justify-center bg-bb-white-00 border-2 border-bb-black-00 border-solid rounded-full transition-all outline-none hover:cursor-pointer hover:shadow-form-hover focus-visible:ring-4 focus-visible:ring-bb-teal-100"
        style="width:3rem;height:3rem;flex-shrink:0;">
      </button>
      <span class="text-[1.25rem] leading-normal text-bb-black-00 cursor-pointer">${label}</span>
    </div>`;
}

// ─── Constants ────────────────────────────────────────────────────────────────

const PARISHES = ['Christ Church','St. Andrew','St. George','St. James','St. John','St. Joseph','St. Lucy','St. Michael','St. Peter','St. Philip','St. Thomas'];
...
function setFieldError(fieldId, message) {
  ...
  if (wrapper) wrapper.classList.replace('border-bb-black-00', 'border-bb-red-00');
...
function clearFieldError(fieldId) {
  ...
  const input = document.getElementById(fieldId);
  const errorEl = document.getElementById(fieldId + '-error');
  if (wrapper) { wrapper.classList.remove('border-bb-red-00'); if (!wrapper.classList.contains('border-bb-black-00')) wrapper.classList.add('border-bb-black-00'); }
...
function showErrorSummary(errors) {
  ...
  errors.forEach(({ fieldId, message }) => {
    ...
    li.innerHTML = `<a href="#${fieldId}" class="underline text-bb-red-00 hover:no-underline">${message}</a>`;
    list.appendChild(li);
  });
