...
- **Phone format:** `XXX-XXXX` (7 digits with hyphen)
...
- Produce a **single self-contained HTML file** with all CSS and JavaScript inline.
- Include the Tailwind CSS CDN and configure the custom design tokens in a `<script>` tag:
  ```html
  ...
  ```
- The prototype must be fully clickable – navigation between pages must work via JavaScript (show/hide sections or hash-based routing).
...
- Include a persistent **header** with the Government of Barbados branding (trident icon + "Government of Barbados" text on `bg-bb-yellow-100` background) and a **footer** on `bg-bb-blue-100` background.
- Include the **alpha banner** on every page: light blue background (`bg-bb-blue-10`) with text "This page is in Alpha." where "Alpha" is a link.
